...
class ParamFileParser(EnumParserBase):
    ...
    element_regex = r'(?:(?:\w[\w\.\s]*)|(?:\".*?\"))'
    name_regex = r'\w[\w\s\:]*'
    pair_regex = (
        r'(?:'
        + name_regex + r'\s*' + r'(?:\=\s*' + element_regex + r'\s*)?'
        + r')'
    )
    array_regex = (
        r'(?:\{\s*'
        + pair_regex + r'(?:\,' + pair_regex + r')*'
        + r'\s*\})'
    )
    qualifier_regex = re.compile(
        r'\s*(?P<name>\w+)(?:\s*=\s*(?P<value>'
        + element_regex + '|' + array_regex
        + r'))?'
        + r'\s*(?:(?:\,\s*)|(?:$))'
        # r'\s*(?P<name>\w+)(?:\s*=\s*(?P<value>'
        # r'(?:\w[\w\.\s]*)|'
    ...
    @classmethod
    def parse_qualifiers(cls, comment):
        qualifiers = {}
        if not comment.startswith('['):
            return qualifiers
        comment = comment.split(']', maxsplit=1)[0].lstrip('[')
        for match in cls.qualifier_regex.finditer(comment):
    ...
    @classmethod
    def parse_line(cls, line, src=None, dont_promote_qualifiers=False):
        member = {}
        rem = line.split(cls.comment, 1)
        if (not rem[0]) or rem[0].isspace():
            return member
        if len(rem) == 2:
            member['doc'] = rem[1].strip()
            if member['doc'].startswith('['):
                member['qualifiers'] = cls.parse_qualifiers(member['doc'])
                if not dont_promote_qualifiers:
                    member.update(member['qualifiers'])
                member['doc'] = member['doc'].split(
                    ']', maxsplit=1)[-1].strip()
        rem = rem[0].split()
    ...
    def parse_lines(self, lines, src=None):
        for line in lines:
            member = self.parse_line(line, src=src)
            if not member:
                continue
...
class CEnumGeneratorHeader(CEnumGeneratorBaseHeader):
    ...
    added_collection_classes = {
        'map': {
            ...
            'enabled_by_controls': {
                'default_value_type': 'std::string',
                'default_value_key': 'CTRL_ON',
                'default_explicit': True,
                'default_packed': True,
            },
            'disabled_by_controls': {
                'default_value_type': 'std::string',
                'default_value_key': 'CTRL_OFF',
                'default_explicit': True,
                'default_packed': True,
            },
            'matlab_var': {
                'default_value_type': 'std::string',
                'default_value_key': 'MATLAB_VAR',
                'default_explicit': True,
                '_disabled': True,
            },
        },
    ...
    def create_child_classes(cls):
        ...
        for ktype, classes in cls.added_collection_classes.items():
            ...
            for k, v in classes.items():
                kcls = base.create_class(k, **v)
                if not kcls._disabled:
                    added[k] = kcls
...
def ePhotosynthesis(args):
    ...
    added_classes = {
        ...
        'units': {
            'explicit': True,
            'singular_collection_name': 'units',
            'no_from_function': True,
        },
        'citations': {
            'explicit': True,
        ...
        'enabled_by_controls': {
            'explicit': True,
            'singular_collection_name': 'enabled_by_control',
            'ragged_right': True,
            'packed': 'std::vector',
        },
        'disabled_by_controls': {
            'explicit': True,
            'singular_collection_name': 'disabled_by_control',
            'ragged_right': True,
            'packed': 'std::vector',
        },
        # 'matlab_var': {
        #     'explicit': True,
        #     'singular_collection_name': 'matlab_var',
        #     'no_from_function': True,
        # },
        'value_flags': {
            'is_editable': True,
