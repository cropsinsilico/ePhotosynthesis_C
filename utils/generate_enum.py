...
class CEnumGeneratorBaseHeader(CEnumGeneratorBase):
    ...
    def generate_item(self, name, members):
        assert self.parent
        return [self.parent.generate_declaration(name, members,
                                                 for_header=True) + ";"]
...
class CEnumGeneratorNameMapBase(CEnumGeneratorBaseSource):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(CEnumGeneratorNameMapBase, self).__init__(*args, **kwargs)
    ...
    def generate_definition(self, name, members, **kwargs):
        ...
        lines += super(CEnumGeneratorNameMapBase, self).generate_definition(
    ...
    def generate_item(self, name, members):
        ...
        if self.namespaces:
            func_name = f"{'::'.join(self.namespaces)}::{func_name}"
        lines += super(CEnumGeneratorNameMapBase, self).generate_item(
            name, members)
        lines += [
            f"template<> const std::map<const {name}, "
            f"const std::string>& get_enum_{self.function_suffix}"
            f"<enum {name}>() {{",
            f"  return {func_name}();",
            "};",
            ""
...
class CEnumGeneratorValueMapSource(CEnumGeneratorNameMapBase):
...
class CEnumGeneratorNameMapSource(CEnumGeneratorNameMapBase):

    name = 'names'
    file_suffix = '_names'
    header_attr = dict(
        CEnumGeneratorNameMapBase.header_attr,
        prefix=CEnumGeneratorNameMapBase.header_attr['prefix'] + [
            '#include <string>',
        ],
    )
...
class CEnumGeneratorHeader(CEnumGeneratorBase):
    ...
    def __init__(self, *args, **kwargs):
        kwargs.setdefault('kwargs_names_header', {})
        kwargs['kwargs_names_header'].setdefault('include_files', [])
        kwargs['kwargs_names_header']['include_files'].append(self)
