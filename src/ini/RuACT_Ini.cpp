...
void RuACT::_initOrig(Variables *theVars, RuACTCondition* RuACT_con) {
    ...
    theVars->RuACT_RC.k1 = 0.006 * theVars->RuACTRatio[0];                // The rate constant of the activation of the Rubisco bound with RuBP. This step is associated with the ARubisco theVars->activase content or activity;
    theVars->RuACT_RC.kn1 = 1.6 * pow(10., -3.) * theVars->RuACTRatio[1]; // The rate constant of E inactivation by binding of RuBP;
    theVars->RuACT_RC.km1 = 20 * pow(10., -6.) * theVars->RuACTRatio[2];  // The michaelis menton constant for RuBP with E.
    theVars->RuACT_RC.Ke2 = 0.1 * theVars->RuACTRatio[3];                 // Data from Mate et al 1996. Unit: micormolar;
    theVars->RuACT_RC.Ke3 = 1.600 * theVars->RuACTRatio[4];               // Data from Mate et al 1996. Unit: micormolar;
    theVars->RuACT_RC.k6 = 2.5 * theVars->RuACTRatio[5] * 4. / 3.;          // micromolar per meter square per second, transfered to unit
    theVars->RuACT_RC.kc = 0.016 * theVars->RuACTRatio[6];                // Michaelis menton constant for CO2
    theVars->RuACT_RC.ko = 0.448 * theVars->RuACTRatio[7];                // Michaelis menton constant for O2
    theVars->RuACT_RC.k7 = theVars->RuACT_RC.k6 * 10 * theVars->RuACTRatio[8]; // The rate constant for ecm to ecmr
    theVars->RuACT_RC.kr = 20. * pow(10., -3.) * theVars->RuACTRatio[9];   // The apparaent michaelis menton constant for RuBP
    ...
    theVars->RuACT_Pool.ET = 0.3 * 4. * factor * theVars->RuACTRatio[11]; //  The total concentraiton of E, ER, EC, ECM, ECMR , mM;
    theVars->RuACT_Pool.Rac = 0.0056 * theVars->RuACTRatio[12];          // The concentration of the activase, mM
...
void RuACT::_initAlt(Variables *theVars, RuACTCondition* RuACT_Con) {
	...
	break;
      switch (it->first) {
      case (RC::RuACT::k6) : {
	it->second = it->second * theVars->RuACTRatio[i] * 4.0 / 3.0;
	i++;
	break;
      }
      case (RC::RuACT::k7) : {
	it->second = it->second * theVars->RuACTRatio[i] * theVars->RuACT_RC[RC::RuACT::k6];
	i++;
	break;
      }
      default : {
	it->second = it->second * theVars->RuACTRatio[i];
	i++;
      }
      }
    }
	...
	break;
      switch (it->first) {
      case (POOL::RuACT::ET) : {
	it->second = it->second * theVars->RuACTRatio[i] * RuACT::get(MOD::RuACT::factor);
	i++;
	break;
      }
      default : {
