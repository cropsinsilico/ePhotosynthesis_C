...
void SUCS::_Rate(const double t, const SUCSCondition* SUCS_Con, Variables *theVars) {
    ...
    if (theVars->useC3) {
        ...
        const double temp55 = Km551 * Km552 * ( 1 + SUCS::UTPc/Km551 + G1Pc/Km552 + SUCS_Con->UDPGc/Km553 + OPOPc/Km554 + SUCS::UTPc * G1Pc/(Km551 * Km552) + SUCS_Con->UDPGc * OPOPc/(Km553 * Km554));
        theVars->SUCS_Vel.v55 = SUCSV55 * (SUCS::UTPc * G1Pc - SUCS_Con->UDPGc * OPOPc/KE55)/temp55;
        ...
        theVars->SUCS_Vel.v57 = SUCSV57 * (SUCS_Con->SUCP - SUC * Pic/KE57)/temp57;
        ...
        const double temp59 = (F6Pc + Km593 * ( 1 + SUCS_Con->F26BPc/Km592)*(1+DHAPc/KI592)) * (SUCS::ATPc + Km591 * (1 + ADPc/KI591));
        theVars->SUCS_Vel.v59 = V59*Vfactor59*Vf_T59 * (SUCS::ATPc * F6Pc - ADPc * SUCS_Con->F26BPc/KE59)/temp59;

        const double temp60 = Km602 * Km603 * (1 + SUCS::ATPc/Km602 + UDPc/Km603 + SUCS::ATPc * UDPc/(Km602 * Km603) + ADPc/Km601 + SUCS::UTPc/Km604 + ADPc * SUCS::UTPc/(Km601 * Km604));
        theVars->SUCS_Vel.v60 = V60 * (SUCS::ATPc * UDPc - ADPc * SUCS::UTPc/KE60 )/temp60;

        theVars->SUCS_Vel.v62 = V62 * SUC/(SUC + Km621);
        ...
        const double temp59 = (F6Pc + Km593) * (SUCS::ATPc + Km591 * (1 + ADPc / KI591));// This is the orginal equation
        ...
        theVars->SUCS_Vel.v55 = V55 * (SUCS::UTPc * G1Pc - SUCS_Con->UDPGc * OPOPc / KE55) / temp55; // G1P+UTP --OPOP+UDPG
        ...
        theVars->SUCS_Vel.v57 = V57 * (SUCS_Con->SUCP - SUC * Pic / KE57) / temp57; // SUCP--Pi + SUC
        theVars->SUCS_Vel.v58 = V58 * SUCS_Con->F26BPc / (temp58 * (1 + Pic / KI582) * (1 + F6Pc / KI583)); // F26BP--F6P + Pi
        theVars->SUCS_Vel.v59 = V59 * (SUCS::ATPc * F6Pc - ADPc * SUCS_Con->F26BPc / KE59) / temp59;     // F6P + ATP --ADP + F26BP
        theVars->SUCS_Vel.v60 = 0.;  // ATP+UDP --UTP + ADP
        theVars->SUCS_Vel.v61 = 0.;  // POPO --2PO
        theVars->SUCS_Vel.v62 = V62 * SUC / (SUC + Km621); // SUC SINK
