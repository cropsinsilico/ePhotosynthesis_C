...
void PR::_Rate(const double t, const PRCondition* PR_con, Variables *theVars) {
    ...
    if (theVars->useC3) {
       ...
       if (theVars->PR_PS_com) {
        ...
        theVars->PR_Vel.v112 = PR::PrV112 * PR_con->PGCA / (PR_con->PGCA + PR::KM112 * (1 + PR_con->GCA / PR::KI1122) * (1 + theVars->Pi / PR::KI1121));

         if (theVars->PR_PS_com) {            // For the combined PS-PR MODEL
             theVars->PR_Vel.v113 = PR::PrV113 * (PR_con->parent->PS_con->ATP * PR_con->GCEA - theVars->ADP * PR_con->parent->PS_con->PGA / PR::KE113) / ((PR_con->parent->PS_con->ATP + PR::KM1131 * (1 + PR_con->parent->PS_con->PGA / PR::KI113)) * (PR_con->GCEA + PR::KM1132));
         } else {
             theVars->PR_Vel.v113 = PR::PrV113 * (PR_con->parent->PS_con->ATP * PR_con->GCEA - theVars->ADP * PR::PGA / PR::KE113) / ((PR_con->parent->PS_con->ATP + PR::KM1131 * (1 + 2.5 / PR::KI113)) * (PR_con->GCEA + PR::KM1132));
         }

        theVars->PR_Vel.v121 = PR::PrV121 * PR_con->GCAc / (PR_con->GCAc + PR::KM121);

        theVars->PR_Vel.v122 = PR::PrV122 * (PR_con->GOAc * PR_con->SERc - PR_con->HPRc * PR_con->GLYc / PR::KE122) / ((PR_con->GOAc + PR::KM1221) * (PR_con->SERc + PR::KM1222 * (1 + PR_con->GLYc / PR::KI1221)));
        theVars->PR_Vel.v123 = PR::PrV123 * (PR_con->HPRc * PR::NADHc - PR_con->GCEA * PR::NADc / PR::KE123) / (PR_con->HPRc + PR::KM123 * (1 + PR_con->HPRc / PR::KI123));
        theVars->PR_Vel.v124 = PR::PrV124 * (PR_con->GOAc * PR::GLUc - PR::KGc * PR_con->GLYc / PR::KE124) / ((PR_con->GOAc + PR::KM1241) * (PR::GLUc + PR::KM1242 * (1 + PR_con->GLYc / KI124)));

        theVars->PR_Vel.v131 = PR::PrV131 * PR_con->GLYc/(PR_con->GLYc + PR::KM1311*(1 + PR_con->SERc / PR::KI1311));
