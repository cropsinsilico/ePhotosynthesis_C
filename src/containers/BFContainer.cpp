...
BFContainer::BFContainer(const BFContainer* other) {
    ISPHr = other->ISPHr;
    cytc1 = other->cytc1;
    ISPo = other->ISPo;
...
BFContainer::BFContainer(const arr &vec, const size_t offset) {
    fromArray(vec, offset);
}

void BFContainer::_fromArray(const arr &vec, const size_t offset) {
    ISPHr = vec[offset];
    cytc1 = vec[offset + 1];
    ISPo = vec[offset + 2];
