...
FIContainer::FIContainer(const FIContainer* other) {
    A = other->A;
    U = other->U;
...
FIContainer::FIContainer(const arr &vec, const size_t offset) {
    _fromArray(vec, offset);
}

void FIContainer::_fromArray(const arr &vec, const size_t offset) {
     A = vec[offset];
     U = vec[offset + 1];
