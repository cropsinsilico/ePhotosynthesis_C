...
EPSCon::EPSCon(const EPSCon* other) {
    _clear();
    CM_con = other->CM_con;
...
EPSCon::EPSCon(realtype *x, const uint adjust) {
    fromArray(x, adjust);
}

EPSCon::EPSCon(FIBFCon* fother, CMCon* cother) {
    _clear();
    CM_con = cother;
    FIBF_con = fother;
    CM_con->setParent(this);
...
EPSCon::EPSCon(const arr &vec, const size_t offset) {
    _fromArray(vec, offset);
}

void EPSCon::_fromArray(const arr &vec, const size_t offset) {
    if (FIBF_con == nullptr)
...
void EPSCon::fromArray(realtype *x, const uint adjust) {
    arr vec = zeros(size());
