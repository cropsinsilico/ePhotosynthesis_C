...
PS_PRCon::PS_PRCon(const PS_PRCon* other) {
    _clear();
    PS_con = other->PS_con;
    PR_con = other->PR_con;
...
PS_PRCon::PS_PRCon(PSCon* sother, PRCon* rother) {
    _clear();
    PS_con = sother;
    PR_con = rother;
    PS_con->setParent(this);
...
PS_PRCon::PS_PRCon(const arr &vec, const size_t offset) {
    _fromArray(vec, offset);
}

void PS_PRCon::_fromArray(const arr &vec, const size_t offset) {
    if (PS_con == nullptr)
