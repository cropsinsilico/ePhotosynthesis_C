function(library_type LIBRARY_FILE OUTPUT_VARIABLE)
  # TODO: Handle .lib on windows which could be static or import
  string(REGEX MATCH "\.(so)|(dylib)|(dll)$" EXT_MATCH ${LIBRARY_FILE})
  if(EXT_MATCH)
    set(LIBRARY_TYPE SHARED)
  else()
    set(LIBRARY_TYPE STATIC)
  endif()
  set(${OUTPUT_VARIABLE} ${LIBRARY_TYPE} PARENT_SCOPE)
endfunction()
...
function(set_conda_prefix)
  ...
  if(DEFINED ENV{CONDA_BUILD})
    cmake_path(SET CONDA_PREFIX "$ENV{PREFIX}")
  else()
  ...
  set(CONDA_PREFIX ${CONDA_PREFIX} PARENT_SCOPE)
endfunction()
...
function(find_package_python)
    ...
    if(ARGS_COMPONENTS)
      list(APPEND FIND_PACKAGE_ARGS COMPONENTS ${ARGS_COMPONENTS})
    endif()
    set(NUMPY_REQUIRED)
    if(ARGS_REQUIRED)
      list(APPEND FIND_PACKAGE_ARGS REQUIRED)
      list(FIND ARGS_COMPONENTS "NumPy" IDX_NumPy)
      if(NOT IDX_NumPy EQUAL -1)
        set(NUMPY_REQUIRED ON)
      endif()
    endif()
    ...
    if(${Python_PREFIX}_EXECUTABLE)
        ...
        if(NUMPY_REQUIRED AND NOT ${Python_PREFIX}_NumPy_INCLUDE_DIRS)
	    execute_process(
    ...
    find_package(${Python_PREFIX} ${FIND_PACKAGE_ARGS})
    if(NUMPY_REQUIRED AND NOT ${Python_PREFIX}_NumPy_FOUND)
        message(FATAL_ERROR "NumPy headers not found")
    endif()
    if(ARGS_REQUIRED AND NOT ${Python_PREFIX}_FOUND)
...
function(find_package_pkgconfig name)
    set(options REQUIRED VERBOSE)
    set(oneValueArgs HEADER LIBRARY_TYPE)
	...
	if ((NOT ${name}_INCLUDE_DIR STREQUAL ${name}_INCLUDE_DIR-NOTFOUND) AND
	    ...
	    # set(create_interface ON)
            library_type(${${name}_LIBRARY} FOUND_LIBRARY_TYPE)
            if(ARGS_LIBRARY_TYPE)
              if(NOT FOUND_LIBRARY_TYPE STREQUAL "${ARGS_LIBRARY_TYPE}")
                message(FATAL_ERROR "Located library is a ${FOUND_LIBRARY_TYPE} library, but a ${ARGS_LIBRARY_TYPE} library was requested")
              endif()
            else()
              set(ARGS_LIBRARY_TYPE ${FOUND_LIBRARY_TYPE})
            endif()
            add_library(${name} ${ARGS_LIBRARY_TYPE} IMPORTED)
    ...
    if (ARGS_REQUIRED AND NOT ${name}_FOUND)
        message(FATAL_ERROR "Could not locate package \"${name}\"")
    endif()
...
function(find_package_zmq)
    ...
    find_package_pkgconfig(ZeroMQ LIBNAMES zmq libzmq libzmq-static
                           HEADER zmq.h ${PKG_CONFIG_ARGS})

    # Force error
    if ((NOT ZeroMQ_FOUND) AND (WIN32 OR ARGS_REQUIRED))
...
function(list_search_directories output)
  ...
  if (NOT (ARGS_NO_DEFAULT_PATH OR (ARGS_NO_CMAKE_PATH AND ARGS_NO_CMAKE_ENVIRONMENT_PATH)))
    set_conda_prefix()
    if(CONDA_PREFIX)
      set(mingw64 ${CONDA_PREFIX})
      set(mingw32 ${CONDA_PREFIX})
      cmake_path(APPEND mingw64
                 ${CONDA_PREFIX} "Library" "mingw-w64")
      cmake_path(APPEND mingw32 ${mingw64} "x86_64-w64-mingw32")
      list(APPEND conda_prefixes ${mingw32} ${mingw64} ${CONDA_PREFIX})
      ...
      endforeach()
      if (NOT ${CONDA_PREFIX} IN_LIST CMAKE_PREFIX_PATH)
