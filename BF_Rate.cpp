...
void BF_Rate(double t, arr &BF_con, arr &BF_Param, varptr &myVars) {
    ...
    const double ISPHr = BF_con[0];	//	The reduced ion sulfer protein (ISPH)
    const double cytc1 = BF_con[1];	//	The oxidized state of cytc1
    const double ISPo = BF_con[2];	//	The oxidized ion sulfer protein (ISP)
    const double ISPoQH2 = BF_con[3];	//	The complex of oxidized ion sulfer protein and reduced quinone
    const double QHsemi = BF_con[4];	//	Semiquinone
    
    const double cytbL = BF_con[5];	//	The oxidized cytbL
    const double Qi = BF_con[6];	//	The binding Quinone
    const double Q = BF_con[7];	//	Quinone
    const double cytbH = BF_con[8];	//	The oxidized form of cytbH
    const double Qn = BF_con[9];	//	Q-
    
    const double Qr = BF_con[10];	//	Q2-
    const double QH2 = BF_con[11];	//	QH2
    const double cytc2 = BF_con[12];	//	oxidized cytc2
    const double P700 = BF_con[13];	//	The reduced state of P700, including both P700 and excited P700
    double ADP = BF_con[14];	//	ADP in stroma
    
    double Pi = BF_con[15];	//	Phosphate in stroma
    const double ATP = BF_con[16];	//	ATP in stroma
    const double Ks = BF_con[17];	//	K ions in stroma
    const double Mgs = BF_con[18];	//	Mg ions in stroma
    const double Cls = BF_con[19];	//	Cl ions in stroma
    
    const double Aip = BF_con[20];	//	The number of photons in peripheral antenna
    const double U = BF_con[21];	//	The number of photons in core antenna
    const double An = BF_con[22];	//	The reduced electron acceptor in PSI
    const double Fdn = BF_con[23];	//	The reduced ferrodoxin
    const double BFHs = BF_con[24];	//	The proton concentration in stroma, put in unit: mol l-1
    
    // BFHl = BF_con[25];	//	The proton concentration in lumen; put in unit: mol l-1// --unused
    const double PHs = BF_con[26];	//	The PH value of stroma
    const double PHl = BF_con[27];	//	The PH value of lumen
    const double NADPH = BF_con[28];	//	The NADPH concentratin in stroma, Unit: mmol l-1
