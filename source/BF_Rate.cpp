...
void BF::BF_Rate(const double t, const BFCon* BF_con, Variables *theVars) {
    ...
    theVars->BF_Vel.Vinc = Iin * theVars->ChlPSI / (theVars->ChlT2 + theVars->ChlPSI) * 95 / 200;  // PPFD absorbed by core antenna of PSI    Unit: micromole m-2 leaf area per second
    theVars->BF_Vel.Vinp = Iin * theVars->ChlPSI / (theVars->ChlT2 + theVars->ChlPSI) * 105 / 200; // PPFD absorbed by peripheral antenna of PSI  Unit: micromole m-2 leaf area per second
